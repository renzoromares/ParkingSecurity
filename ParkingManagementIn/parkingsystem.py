...
def checkPlateNumber(val):
    ...
    try:
        cursor.execute("SELECT VEHICLE_OWNER.Id_Number, CREDENTIALS.PlateID, VEHICLE_OWNER.Position, CREDENTIALS.Vehicle_Type FROM VEHICLE_OWNER JOIN CREDENTIALS ON CREDENTIALS.Id_Number = VEHICLE_OWNER.Id_Number WHERE CREDENTIALS.PlateID = '" + val + "' and CREDENTIALS.Status = 1" )
        ...
        while True:
            ...
            for encodes, faces in zip(encodeCurrent, facesCurrent):
                ...
                if matches[index]:
                    ...
                    timelogIn.createLog(PlateID)
                    parkingslotEnter.enterPremise(position, Vehicle_Type)
                    main()
